...
## Fix pack 11 — counts, not conclusions (2026-09-20)

Tier 2 and Tier 3: what the family can see about how the last month went, and one optional tap from
the person themselves.

**What was already built.** `src/lib/trends.ts` and `TrendLines` (Circle Board, Evidence Inspector)
answer "how is one activity going, at one comparable difficulty-and-cue condition" and refuse to
blend across a change of either. That is a different question from "did they take part, and did the
reminders get answered", which needs no such control — so `src/lib/participation.ts` is new rather
than a rewrite of trends, and the existing screens are untouched.

**2.2 — the status strip.** One line at the top of Circle, picked from five fixed translated
sentences (`strip.*`), never assembled from parts, with an optional second clause when reminder
answering is down on this person's own previous week. The comparison is always this person against
themselves: `participation()` takes no population anything.

**2.1 — thirty days.** One bar per day (30 divs, 4 px wide at 360 px — no chart library), days
joined this week against the week before, sessions finished with no help, and the week's reminder
answers. Under `MIN_DAYS_FOR_COMPARISON` days across the two weeks, the comparison is replaced by
`trend.too_little` and no line is drawn.

**2.3 — the mood tap.** Three faces on the completion screen in `GameFrame`, so all seven activities
get it from one place. It writes `mood` onto the trial row already logged (`db.trials.update`, not a
second row, and not indexed — no Dexie version bump). Optional by construction: it appears after the
activity is over, walking away is how it is skipped, so there is no extra "Skip" control for an
elder to read, and `model.ts` never sees it.

**Tier 3 — how today went.** Counts of the day's own trial and reminder rows, in the same card. No
template chooses an adjective and nothing is generated by a model. With nothing recorded, it says so.

**A real bug the tests caught:** the card rendered English on an Assamese profile whenever Circle was
the first screen opened. `t()` answers from the English fallback until `loadManifest` resolves, and
the card read `getPerson()` directly instead of `preparePerson()`, which is what loads the manifest.

Strings: 26 new keys through the pipeline. Two failed and were fixed by rewriting the English —
`trend.each_bar` ("Each bar is one day." → "Each bar shows one day.", 0.39 → 0.88) — and
`mood.thanks` was deleted rather than reworded: the selected face is its own acknowledgement.
`mood.*` is spoken like the rest of the elder's strings; the caregiver counts are text-only.
