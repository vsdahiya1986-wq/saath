...
- [x] **Saah Pat · Tea Leaf** (F4, Sept 2026) — attention: find the "two leaves and a bud" sprigs. 6/9/12/16 tiles, reveal after `targets + 4` wrong taps, 2 rounds, logged as `saah_pat`. e2e completes in EN and AS.
- [x] **Apon Mukh · Dear Faces** (F5, Sept 2026) — memory card-matching on approved Memory Garden photos, regional objects as fallback. 3–6 pairs, reveal after `pairs × 4` mismatches, logged as `apon_mukh`. e2e completes in EN and AS.
- [x] **Aajir Tini · Today's Three** (F6, Sept 2026) — three least-recently-played activities from three domains, above "All activities" on the Play screen.
...
- [x] **Assamese audio regenerated** — 81/81 strings, both languages (Sept 2026). `activity.saah_pat` needs native-speaker review (gloss translated twice).
